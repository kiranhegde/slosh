subroutine check_time
    ...
    dt=min(min(dt,0.05_dp/(Cdmax+1.e-10_dp)),dt)
...
subroutine check_ns
    ...
    iter=1
    do while(residual>mass.and.iter<=itermax)!when residual<mass stop and.iter<=5
    ...
    if(residual>mass.or.residual>10.*residualold) then
        time=time-dt
        dt=0.5*dt
        time=time+dt
        ...
        !end of recompute the answer
    end if 
    
    residualold=residual

    if(residual<mass/100.) then
        dt=2.0*dt
    end if 
