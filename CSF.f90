...
subroutine csf
    ...
    do i=RS,RE
            do k=ZS,ZE
                W1=sqrt(FN(i,k)*(1._dp-FN(i,k)))
                W2=sqrt(FN(i-1,k)*(1._dp-FN(i-1,k)))
                H1=1.0/(1.0-CFc)*(min(max(FN(i,k),CFc/2.),1.-CFc/2.)-CFc/2.)
                H2=1.0/(1.0-CFc)*(min(max(FN(i-1,k),CFc/2.),1.-CFc/2.)-CFc/2.)
                delta=(H1-H2)/dr
...
subroutine curvature_compute
    ...
    do i=RS,RE
        do k=ZS,ZE
            ...
            Fft=Nnormalfz(i,k+1)
            !boundary condition
            if(0.<F(i,k).and.F(i,k)<1.and.i==RE) then
                !sign=sgn(U(i,k))   !-et.U(i,k) only consider about the direction
                !theta=0.5_dp*((thetaA+thetaM)+(thetaA-thetaM)*sign)
                theta=89./180.*pi
                Ffe=Ffe*sin(theta)+cos(theta) 
                !n^=N^*sin(theta)-cos(theta)*nwall,
                !Ffw,Ffe,Ffb,Fft is the compenete of nf
            end if
